...
自然言語で Azure の確認作業を依頼し、AI が Azure CLI を実行する体験をします。最後に、GitHub Enterprise Cloud に push する簡単なアプリを作るお題に取り組み、プロンプトや設計の違いで成果物が変わることを体感します。

目安時間: Azure 確認 15分、アプリ作成 25分、共有 10分
...
1. 最初は読み取り専用の確認作業だけを依頼します。
    - 対象リソースグループにある VM の一覧を確認してください。
    - VNet、Subnet、NSG、NAT Gateway の関係を確認してください。
...
- 実行結果をもとに、この環境のネットワーク構成を説明してください。

## GitHub に push する簡単なアプリを作る

ここからは、同じお題をもとに各自でアプリを作ります。全員が同じものを作る必要はありません。プロンプト、設計、利用する言語やフレームワークによって、成果物が変わることを体感します。
...
1. SRE Agent に、運用監視や障害調査の観点で不足している設定を相談します。
